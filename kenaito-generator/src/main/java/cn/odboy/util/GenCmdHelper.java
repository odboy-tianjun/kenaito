...
public class GenCmdHelper {
    ...
    public void gen(String tablePrefix, List<String> tableNames) {
        // 1.数据库配置
        DataSourceConfig.Builder dataSourceConfigBuilder = new DataSourceConfig.Builder(this.databaseUrl, this.databaseUsername, this.databasePassword);
        ...
        fastAutoGenerator.packageConfig(packageConfigBuilder -> packageConfigBuilder
                .parent(this.parentPackageName == null ? "cn.odboy" : this.parentPackageName)
                .moduleName(this.parentPackageModuleName == null ? "newModule" : this.parentPackageModuleName)
                .entity("domain")
                .service("service")
                .mapper("mapper")
